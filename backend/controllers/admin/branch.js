...
async function getNextBranchId(req, res) {
  try {
    res.json({
      status: true,
      data: await branchService.getNextBranchId(),
    });
  } catch (err) {
    res.json({
      status: false,
...
module.exports = { find, findById, getState, create, update, remove, getNextBranchId };
