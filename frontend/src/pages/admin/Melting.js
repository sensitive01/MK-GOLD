                  ...
                  <Card variant="outlined" sx={{ p: 2, bgcolor: '#fafafa', borderRadius: 1.5 }}>
                    ...
                    {isPdf ? (
                      <Box
                        ...
                        src={fullUrl}
                        title={title}
                        sx={{ width: '100%', height: 350, border: '1px solid #e0e0e0', borderRadius: 1 }}
                      />
                    ) : (
                      <Box
                        component="img"
            ...
            return (
              <Grid container spacing={2.5}>
                ...
                <Grid item xs={12} md={6}>
                  ...
                  <Typography variant="subtitle2">Estimated Gross Weight (g)</Typography>
                  <Typography variant="body2" gutterBottom>{rowToView.totalGrossWeight}</Typography>

                  <Typography variant="subtitle2">Estimated Net Weight (g)</Typography>
                  <Typography variant="body2" gutterBottom>{rowToView.totalNetWeight}</Typography>
                </Grid>

                {rowToView.notes && (
                  <Grid item xs={12}>
                    <Typography variant="subtitle2">Initial Notes</Typography>
                    <Typography variant="body2">{rowToView.notes}</Typography>
                  </Grid>
                )}

                {/* Stage 1: Pre-Melt Verification */}
                {(rowToView.isPreMeltCompleted || rowToView.actualGrossWeight != null || rowToView.actualNetWeight != null || rowToView.actualStoneWastage != null || rowToView.preMeltProof) && (
                  <>
                    <Grid item xs={12}>
                      <Divider sx={{ my: 1 }} />
                    ...
                    <Grid item xs={12} sm={4}>
                      <Typography variant="subtitle2">Actual Gross Weight (g)</Typography>
                      <Typography variant="body2" gutterBottom>{rowToView.actualGrossWeight ?? 'N/A'}</Typography>
                    </Grid>
                    <Grid item xs={12} sm={4}>
                      <Typography variant="subtitle2">Actual Net Weight (g)</Typography>
                ...
                {/* Stage 2: Melt Results */}
                {(rowToView.status === 'melt_updated' || rowToView.status === 'sold' || rowToView.barWeight != null || rowToView.afterMeltProof) && (
                  <>
                    <Grid item xs={12}>
                      <Divider sx={{ my: 1 }} />
                      <Typography variant="subtitle1" sx={{ color: 'primary.main', fontWeight: 700, mb: 1 }}>
                    ...
                    <Grid item xs={12} sm={6}>
                      ...
                      <Typography variant="body2" gutterBottom>{rowToView.barWeight ?? 'N/A'}</Typography>
                    </Grid>
                    <Grid item xs={12} sm={6}>
                      <Typography variant="subtitle2">Final Bar Purity (%)</Typography>
                      <Typography variant="body2" gutterBottom>{rowToView.barPurity ?? 'N/A'}</Typography>
                    </Grid>
                    {rowToView.weightDifference != null && (
                      <Grid item xs={12} sm={6}>
                        <Typography variant="subtitle2">Weight Difference</Typography>
                        <Typography variant="body2" gutterBottom sx={{ color: rowToView.weightDifference < 0 ? 'error.main' : 'success.main', fontWeight: 600 }}>
                          {rowToView.weightDifference > 0 ? '+' : ''}{rowToView.weightDifference} g
                        </Typography>
                      </Grid>
                    )}
                    {rowToView.purityDifference != null && (
                      <Grid item xs={12} sm={6}>
                        <Typography variant="subtitle2">Purity Difference</Typography>
                        <Typography variant="body2" gutterBottom sx={{ color: rowToView.purityDifference < 0 ? 'error.main' : 'success.main', fontWeight: 600 }}>
                          {rowToView.purityDifference > 0 ? '+' : ''}{rowToView.purityDifference}%
                        </Typography>
                      </Grid>
                    )}
                    {rowToView.meltUpdateNotes && (
                      <Grid item xs={12}>
                        <Typography variant="subtitle2">Melt Update Notes</Typography>
                ...
                {/* Uploaded Proofs */}
                {(rowToView.meltProof || rowToView.preMeltProof || rowToView.afterMeltProof) && (
                  <Grid item xs={12}>
                    <Divider sx={{ my: 1 }} />
                ...
                {renderProofCard('Initial Batch Proof (Batch Creation)', rowToView.meltProof)}
                {renderProofCard('Pre-Melt Proof (Stage 1 Update)', rowToView.preMeltProof)}
                {renderProofCard('After Melt Proof (Stage 2 Final Bar)', rowToView.afterMeltProof)}
              </Grid>
            );
          })()}
        </DialogContent>
