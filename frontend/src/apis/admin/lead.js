...
async function getLeads(query = {}) {
  try {
    const response = await apiClient().post('/api/v1.0/admin/lead/get', query);
...
async function createLead(data) {
  try {
    const response = await apiClient().post('/api/v1.0/admin/lead/create', data);
    return response.data;
  } catch (err) {
    return err;
  }
...
async function getLeadStats() {
  try {
    const response = await apiClient().get('/api/v1.0/admin/lead/get-stats');
    return response.data;
  } catch (err) {
    return err;
  }
